...
(function(root, factory) {
  if (typeof exports === 'object' && typeof exports.nodeName !== 'string') {
    ...
    // Browser globals
    root.gamebank = root.gamebank || {};
    factory((root.gamebank.crypto = {}));
  }
  ...
  function PrivateKey(priv, pub) {
    ...
    this.sign = function(hash) {
      return fromBits(sjcl.codec.gamebank.signRecoverably(priv, toBits(hash)));
    };
  }

  PrivateKey.from = function(wif, header) {
    return new PrivateKey(
      sjcl.codec.gamebank.deserializePrivateKey(wif, header)
    );
  };
  ...
  PublicKey.from = function(str) {
    return new PublicKey(sjcl.codec.gamebank.deserializePublicKey(str));
  };

  PublicKey.recover = function(hash, sig) {
    return new PublicKey(
      sjcl.codec.gamebank.recoverPublicKey(toBits(hash), toBits(sig))
    );
  };

  PublicKey.prototype = {
    toString: function() {
      return sjcl.codec.gamebank.serializePublicKey(this._p);
    },
  ...
  function keysFromPassword(accountName, accountPassword) {
    var keys = sjcl.codec.gamebank.keysFromPassword(
      accountName,
      accountPassword
    );
    return {
  ...
  function serializePair(k) {
    return {
      private: sjcl.codec.gamebank.serializePrivateKey(k.sec),
      public: sjcl.codec.gamebank.serializePublicKey(k.pub)
    };
  }
