...
hashes and encodings required by the gamebank platform.
...
Otherwise, just
```sh
$ yarn install gamebank-crypto
```

## API

```
> crypto = require('gamebank_libcrypto');
```
...
Provides operations over gamebankit secp256k1-based ECC private keys.
```
...
Provides operations over gamebankit secp256k1-based ECC public keys.
```
...
Generates a new pair of keys in gamebank WIF format using cryptographically secure
...
Given a gamebankit account name and password, regenerates the derived `owner`, `posting`,
...
gamebank-crypto is written in Javascript as specified by 
...
gamebank-crypto explicitly supports the following environments without polyfills:
...
gamebank-crypto explicitly does not support the following environments:
- Opera Mini
